...
class NaiveAgent:
    ...
    def _make_move(self):
        ...
        if self._turn_count == 1:
            ...
            value = self.MinMaxParallel(newBoard, True, self.depth, -math.inf, math.inf)
    ...
    def MinMaxParallel(self, startBoard, maximizingPlayer, depth, alpha, beta):
        pool = Pool(processes=4)  # You can adjust the number of processes based on your machine's capabilities
        moves = self.get_moves_ordered(startBoard)

        if maximizingPlayer:
            scores = pool.starmap(
                self.parallel_min_max,
                [(copy.deepcopy(startBoard), False, depth - 1, alpha, beta, move) for move in moves],
            )
            best_score_index = np.argmax(scores)
            self._best_move = moves[best_score_index]
            return scores[best_score_index]
        else:
            scores = pool.starmap(
                self.parallel_min_max,
                [(copy.deepcopy(startBoard), True, depth - 1, alpha, beta, move) for move in moves],
            )
            best_score_index = np.argmin(scores)
            self._best_move = moves[best_score_index]
            return scores[best_score_index]

    def parallel_min_max(self, startBoard, maximizingPlayer, depth, alpha, beta, move):
        newBoard = self.update_board(copy.deepcopy(startBoard), move[0], move[1], self._colour)
        if maximizingPlayer:
            return self.MinMax(newBoard, False, depth, alpha, beta, move)
        else:
            return self.MinMax(newBoard, True, depth, alpha, beta, move)
    ...
    def MinMax(self, startBoard, maximizingPlayer, depth, alpha, beta, currentMove):
        ...
        transposition_key = np.array2string(self._board)
    ...
    def CheckSwap(self):
        ...
        if ((moves[0] > swap_border and moves[0] < (self._board_size + swap_border)) and (
                moves[1] > swap_border and moves[1] < (self._board_size + swap_border))):
            return True
        else:
            return False
