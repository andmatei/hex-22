...
class NaiveAgent():
    ...
    def dijkstra_update(self, color, scores, updated):
        ...
        for i in range(self._board_size):
            for j in range(self._board_size):
                if not updated[i][j]:
                    neighbor_coords = self.get_neighbors((i, j))
                    for neighbor_coord in neighbor_coords:
                        target_coord = tuple(neighbor_coord)
                        path_cost = BAD if not self.is_empty(target_coord) else 1
                        if scores[target_coord] > scores[i][j] + path_cost:
                            scores[target_coord] = scores[i][j] + path_cost
                            updated[target_coord] = False
                            heapq.heappush(priority_queue, (scores[target_coord], target_coord))

        while priority_queue:
            current_cost, current_coord = heapq.heappop(priority_queue)
            if not updated[current_coord]:
    ...
    def CheckSwap(self):
       # moves = np.where(self._board == self.opp_color())
      #  moves = list(zip(moves[0], moves[1]))[0]
      #  swap_border = math.sqrt(self._board_size)
      #  return any((swap_border < move < (self._board_size + swap_border)) for move in moves)
